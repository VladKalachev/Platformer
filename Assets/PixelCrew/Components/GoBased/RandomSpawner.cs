...
namespace PixelCrew.Components.GoBased
{
    public class RandomSpawner : MonoBehaviour
    {
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Handles.DrawWireArc(position, Vector3.forward, rightBound, _sectorAngle, 10);
        }

        private Vector2 AngleToVectorInSector(float angle)
        {
            var angleMiddlta = (180 - _sectorRotation - _sectorAngle) / 2;
            return GetUnitOnCircle(angle + angleMiddlta);
        }

        private Vector3 GetUnitOnCircle(float angleMiddlta)
        {
            var angleRadians = angleMiddlta * Mathf.PI / 180.0f;
